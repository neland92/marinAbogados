...
- [x] **Gestión de Asignaciones (UI):** Se agregó la visualización del responsable asignado a cada ticket en la tabla general. En la vista de detalles, dentro del recuadro "Información", se implementó un menú desplegable (Combo Box) que permite reasignar el ticket a los diferentes roles, registrando automáticamente el cambio en el historial de auditoría. Esta opción de reasignación es exclusiva para el personal de la firma; el rol "Cliente" solo puede ver a quién está asignado el ticket sin poder modificarlo.
